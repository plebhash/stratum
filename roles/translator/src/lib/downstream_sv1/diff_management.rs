...
use super::error::{TProxyDownstreamError, TProxyDownstreamResult};
...
impl Downstream {
    ...
    pub async fn init_difficulty_management(
        ...
        let (connection_id, upstream_difficulty_config, miner_hashrate) = self_
            ...
            .map_err(|_e| TProxyDownstreamError::PoisonLock)?;
    ...
    pub fn remove_miner_hashrate_from_channel(self_: Arc<Mutex<Self>>) -> TProxyDownstreamResult<'static, ()> {
        self_
            .safe_lock(|d| {
    ...
    pub async fn try_update_difficulty_settings(
        ...
        let prev_target = match roles_logic_sv2::utils::hash_rate_to_target(
            ...
            Err(v) => return Err(TProxyDownstreamError::Target(v)),
        };
    ...
    #[allow(clippy::result_large_err)]
    pub fn hash_rate_to_target(self_: Arc<Mutex<Self>>) -> TProxyDownstreamResult<'static, Vec<u8>> {
        self_
            .safe_lock(|d| {
                match roles_logic_sv2::utils::hash_rate_to_target(
                    ...
                    Err(e) => Err(TProxyDownstreamError::Target(e)),
                }
            })
            .map_err(|_e| TProxyDownstreamError::PoisonLock)?
    }
    ...
    #[allow(clippy::result_large_err)]
    pub(super) fn save_share(self_: Arc<Mutex<Self>>) -> TProxyDownstreamResult<'static, ()> {
        self_
            .safe_lock(|d| {
    ...
    #[allow(clippy::result_large_err)]
    pub(super) fn get_set_difficulty(target: Vec<u8>) -> TProxyDownstreamResult<'static, json_rpc::Message> {
    ...
    #[allow(clippy::result_large_err)]
    pub(super) fn difficulty_from_target(mut target: Vec<u8>) -> TProxyDownstreamResult<'static, f64> {
    ...
    pub fn update_miner_hashrate(
        self_: Arc<Mutex<Self>>,
        miner_target: Vec<u8>,
    ) -> TProxyDownstreamResult<'static, Option<f32>> {
        self_
            .safe_lock(|d| {
                let timestamp_secs = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
