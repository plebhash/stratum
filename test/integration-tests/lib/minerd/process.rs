...
fn get_minerd_filename(os: &str, arch: &str) -> Result<String, MinerdError> {
    match (os, arch) {
        ...
        ("linux", "x86_64") => Ok(format!(
            "pooler-cpuminer-{VERSION_MINERD}-x86_64-linux-gnu.tar.gz"
        )),
        ("linux", "aarch64") => Ok(format!(
            "pooler-cpuminer-{VERSION_MINERD}-aarch64-linux-gnu.tar.gz"
        )),
...
impl MinerdProcess {
    ...
    pub async fn new(
        ...
        if !minerd_dir.exists() {
            ...
            let download_endpoint = format!(
                "https://github.com/plebhash/cpuminer/releases/download/v{VERSION_MINERD}/"
            );
    ...
    pub async fn spawn_minerd(
        ...
        // Wait for everything on the test environment to be ready
    ...
    pub async fn measure_hashrate(&self) -> Result<f64, MinerdError> {
        ...
        let stderr = child.stderr.take().ok_or_else(|| {
            MinerdError::ProcessSpawn(std::io::Error::new(
...
/// Measures the hashrate of the local minerd binary in benchmark mode
/// Returns the hashrate in hashes per second
pub async fn measure_hashrate() -> Result<f64, MinerdError> {
    info!("Starting hashrate measurement using minerd benchmark mode");

    let mut cmd = TokioCommand::new("./minerd/minerd");
